...
struct HTTPRequest http_request_constructor(char *request_string) {
	...
	char *requested =
	    (char *)malloc(sizeof(char) * (strlen(request_string) + 1));
...
void extract_request_line_fields(struct HTTPRequest *request,
	...
	struct Dictionary request_line_dict =
	    dictionary_constructor(compare_string_keys);
	// must be sizeof(char) * (strlen(¡¤) + 1)
	request_line_dict.insert(&request_line_dict, "method", sizeof("method"),
	                         method, sizeof(char) * (strlen(method) + 1));
	request_line_dict.insert(&request_line_dict, "uri", sizeof("uri"), uri,
	                         sizeof(char) * (strlen(uri) + 1));
	request_line_dict.insert(&request_line_dict, "http_version",
	                         sizeof("http_version"), http_version,
	                         sizeof(char) * (strlen(http_version) + 1));
	...
	while (header) {
		...
		if (value) {
			...
			request->header_fields.insert(
			    &request->header_fields, key, sizeof(char) * (strlen(key) + 1),
		...
		if (strcmp(content_type, "application/x-www-form-urlencoded") == 0) {
			...
			while (field) {
				...
				body_fields.insert(&body_fields, key,
				                   sizeof(char) * (strlen(key) + 1), value,
			...
			if (json == NULL) {
				...
				while (item) {
					...
					if (cJSON_IsString(item) && item->valuestring) {
						...
						// Convert non-string JSON values (like numbers,
						// booleans, arrays) to strings
						value = cJSON_Print(item);
					}

					if (key && value) {
						body_fields.insert(
						    &body_fields, key, sizeof(char) * (strlen(key) + 1),
						    value, sizeof(char) * (strlen(value) + 1));

						// If value is dynamically allocated (like cJSON_Print),
						// free it after use
			...
			body_fields.insert(&body_fields, "data", sizeof("data"), body,
			                   sizeof(char) * (strlen(body) + 1));
		}
