...
void update_height(void* _n) {
    ...
    if (node != NULL) {
        ...
        node->height = (prev_height > next_height ? prev_height : next_height) + 1;
    }
}
...
// 插入操作，并返回新的子树根
struct TreeNode* insert_node_avl(struct BinarySearchTree* tree, struct TreeNode* node, void* data, int size) {
    // 1. 普通的BST插入
    if (node == NULL)
...
// 删除节点，并返回新的子树根
struct Node* remove_node_avl(struct BinarySearchTree* tree, struct TreeNode* node, void* data, int data_size) {
    // 标准BST删除过程
    if (node == NULL)
        return node;
    ...
    if (cmp < 0) {
        // 要删除的数据在左子树
        node->node.prev = remove_node_avl(tree, node->node.prev, data, data_size);
    }
    else if (cmp > 0) {
        // 要删除的数据在右子树
        node->node.next = remove_node_avl(tree, node->node.next, data, data_size);
    }
    else {
        // 找到要删除的节点
        if (node->node.prev == NULL || node->node.next == NULL) {
            // 一个或零个子节点
            struct TreeNode* temp = node->node.prev ? node->node.prev : node->node.next;
            ...
            // 删除中序后继节点
            node->node.next = remove_node_avl(tree, node->node.next, temp->node.data, data_size);
        }
    }
