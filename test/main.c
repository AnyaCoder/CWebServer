...
{
	char* page = render_template(
		"D:/CppProject/index.html",
...
char* about(struct HTTPServer* server, struct HTTPRequest* request)
{
	...
	{
		char* res = request->body.search(&request->body, "name", sizeof("name"));
		if (res) {
...
int main()
{
	...
	const char* host = "127.0.0.1"; // or "0.0.0.0" to bind to all available interfaces
