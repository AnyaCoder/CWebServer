...
struct Route {
	...
	char *uri;
	char *(*route_function)(struct HTTPServer *server,
	                        struct HTTPRequest *request);
};

struct HTTPServer *http_server_constructor(const char *host, int port) {
	struct HTTPServer *server =
	    (struct HTTPServer *)malloc(sizeof(struct HTTPServer));
	// Convert the host to an IP address
	u_long ip_addr = inet_addr(host);
	server->server =
	    server_constructor(AF_INET, SOCK_STREAM, 0, ip_addr, port, 255);
...
void register_routes(struct HTTPServer *server,
	...
	server->routes.insert(&server->routes, uri,
	                      sizeof(char) * (strlen(uri) + 1), &route,
	                      sizeof(route));
}
...
void launch(struct HTTPServer *server) {
	...
	while (1) {
		...
		client_server->client =
		    accept(server->server->socket, sock_addr, &address_length);
...
void send_file_response(int client, const char *file_path) {
	...
	if (file != NULL) {
		...
		// 计算响应头的大小并生成
		char header[256];
		snprintf(header, sizeof(header), header_template, content_type,
		         file_size);
...
void *handler(void *arg) {
	...
	char *method = (char *)request.request_line.search(
	    &request.request_line, "method", sizeof("method"));
	char *uri = (char *)request.request_line.search(&request.request_line,
	                                                "uri", sizeof("uri"));
...
void replace_variable(char *buffer, size_t buffer_size, const char *placeholder,
                      const char *value) {
